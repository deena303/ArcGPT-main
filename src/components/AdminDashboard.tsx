...
export const AdminDashboard: React.FC<AdminDashboardProps> = ({
  ...
  // AI Configuration toggles (in-memory state from system_settings)
  ...
  const fetchData = () => {
    ...
    Promise.all([
      ...
      fetch('/api/connection/status', { credentials: 'include' }).then(r => r.json()),
      fetch('/api/admin/ai-config', { credentials: 'include' }).then(r => r.ok ? r.json() : null),
    ])
      .then(([statsData, usersData, logsData, historyData, secEventsData, analyticsData, connData, aiCfgData]) => {
        setStats(statsData);
        setUsers(usersData || []);
        ...
        if (aiCfgData) {
          setAiConfig((prev: any) => ({ ...prev, ...aiCfgData }));
        }
        setLoading(false);
      })
      .catch(err => {
  ...
  const handleAddUserSubmit = async (e: React.FormEvent) => {
    e.preventDefault();
    if (!newUserName.trim() || !newUserEmail.trim() || newUserPassword.length < 12) return;
    try {
  ...
  const adminNavItems: { id: AdminTab; label: string; icon: any; count?: number }[] = [
    ...
    { id: 'roles', label: '3. Roles & Permissions', icon: Key },
    { id: 'schema', label: '4. Database Schema', icon: Database },
    { id: 'ai_config', label: '5. AI Configuration', icon: Cpu },
    { id: 'security', label: '6. Security & Query Guard', icon: ShieldAlert },
    { id: 'monitor', label: '7. Query Monitor', icon: Eye, count: queryHistory.length },
    { id: 'audit', label: '8. Audit Logs', icon: FileText, count: auditLogs.length },
    { id: 'analytics', label: '9. System Analytics', icon: BarChart3 },
    { id: 'settings', label: '10. System Settings', icon: SettingsIcon },
  ];
          ...
          <div className="grid grid-cols-1 sm:grid-cols-3 gap-4">
            <div className="rounded-2xl border border-emerald-200 bg-emerald-50/50 p-4 flex items-center justify-between">
              <div>
                ...
                <p className="text-[11px] text-emerald-700 mt-0.5">Model: qwen2.5-coder-7b-instruct</p>
              </div>
            ...
            <div className="space-y-4">
              <div className="rounded-2xl border border-slate-200 bg-slate-50/70 p-4 space-y-3">
                ...
                <div className="p-3 rounded-xl bg-purple-50 border border-purple-200 text-xs text-purple-900">
                  ...
                  <span>
                    Local Ollama configuration is isolated on the Express server.
                  </span>
                </div>
              </div>
            </div>
      ...
      {activeTab === 'security' && (
        <div className="space-y-6">
          <div className="rounded-3xl border border-slate-200 bg-white p-6 shadow-xs space-y-4">
            ...
            {/* Live Security Events Table (public.security_events) */}
            <div className="space-y-3 pt-3">
              <div className="flex items-center justify-between">
                <div>
                  <h3 className="text-sm font-bold text-slate-900 flex items-center gap-2">
                    <ShieldAlert className="h-4 w-4 text-red-600" />
                    <span>Live Security Events Log (public.security_events)</span>
                  </h3>
                  <p className="text-xs text-slate-500">
                    Real-time feed of intercepted malicious commands, RBAC violations, and DDL/DML injection blocks.
                  </p>
                </div>
                <span className="rounded-full bg-red-100 text-red-800 border border-red-200 px-3 py-1 text-xs font-bold">
                  {securityEvents.length} Events Tracked
                </span>
              </div>
                    ...
                    {securityEvents.length === 0 ? (
                      <tr>
                        <td colSpan={6} className="px-4 py-6 text-center text-slate-400 italic">
                          No security violations detected. Query Guard is active and monitoring.
                        </td>
                      </tr>
                    ) : (
                      securityEvents.map((evt, idx) => (
                        <tr key={evt.event_id || idx} className="hover:bg-slate-50/70 transition-colors">
                          ...
                          <td className="px-4 py-3 font-medium text-slate-700">
                            {evt.user_name || evt.user_id || 'System User'}
                          </td>
                          <td className="px-4 py-3 text-slate-600 max-w-xs">
                            <div className="line-clamp-2">{evt.description}</div>
                          </td>
                          <td className="px-4 py-3">
                            <span className="inline-flex items-center gap-1 rounded-full bg-red-50 text-red-700 px-2 py-0.5 text-[10px] font-bold border border-red-200">
          ...
          <div className="rounded-2xl border border-slate-200 bg-slate-50/70 p-5 space-y-4">
            ...
            <div className="grid grid-cols-1 sm:grid-cols-2 gap-4 text-xs">
              <div>
                ...
                <input
                  type="text"
                  defaultValue="ArcGPT Institute of Technology"
